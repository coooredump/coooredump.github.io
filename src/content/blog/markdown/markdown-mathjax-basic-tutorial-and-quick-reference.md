...
用 Katex 改写数学公式，发现右括号接下标，不能正常解析，`)`改成 `\rparen`, `]`改成 `\rbrack`
...
通常使用 `\frac {分子} {分母}` 命令产生一个分数，分数可嵌套。 便捷情况可直接输入 `\frac{a}{b}` 来快速生成一个 。 如果分式很复杂，亦可使用 `分子 \over 分母` 命令，此时分数仅有一层。
...
注意比较两个式子间 `dx` 的不同。 使用 `\operatorname` 命令也可以达到相同的效果，详见 [定义新的符号 `\operatorname`](https://freeopen.github.io/mathjax/#15-qi-ta-ming-ling) 。
...
使用 `\color{颜色}{文字}` 来更改特定的文字颜色。 更改文字颜色需要浏览器支持，如果浏览器不知道你所需的颜色，那么文字将被渲染为黑色。
...
在公式内使用 `\require{cancel}` 来允许 _片段删除线_ 的显示。 声明片段删除线后，使用 `\cancel{字符}`、`\bcancel{字符}`、`\xcancel{字符}` 和 `\cancelto{字符}` 来实现各种片段删除线效果。
